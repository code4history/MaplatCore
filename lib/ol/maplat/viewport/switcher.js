...
const thetas = [0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75].map((pow) => pow * Math.PI);
...
function switcher(fromCenter, fromRotation, fromResolution, baseRadius, fromProj, toProj, baseProj = 'EPSG:3857') {
...
function maplat2Base(maplatCenter, maplatRotation, maplatResolution, baseRadius, maplatProj, baseProj = 'EPSG:3857') {
...
function base2Maplat(baseCenter, baseRotation, baseResolution, baseRadius, maplatProj, baseProj = 'EPSG:3857') {
    const maplatParams = base2MaplatParams(baseCenter, baseRadius, maplatProj, baseProj);
...
function base2MaplatParams(center, radius, maplatProj, baseProj) {
    const maplatVicinities = center2Vicinities(center, radius).map((baseCoord) => transform(baseCoord, baseProj, maplatProj));
