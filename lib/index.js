...
export class MaplatApp extends EventTarget {
    constructor(appOption) {
        ...
        this.logger = new Logger(appOption.debug ? LOGGER_LEVEL.ALL : LOGGER_LEVEL.INFO);
    ...
    handleGPS(launch, avoidEventForOff) {
        ...
        if (launch) {
            ...
            const source = this.mapObject.getLayers().item(0).getSource();
            source.setGPSMarker(null);
            if (!avoidEventForOff)
                this.dispatchEvent(new CustomEvent("gps_result", { error: "gps_off" }));
        }
    }
    handleGPSResults(type) {
        const overlayLayer = this.mapObject.getLayer("overlay").getLayers().item(0);
        const firstLayer = this.mapObject.getLayers().item(0);
