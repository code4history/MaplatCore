...
export class MaplatApp extends EventTarget {
    constructor(appOption) {
        ...
        this.stateBuffer = {};
        this.mobileMapMoveBuffer = undefined;
        this.overlay = true;
        this.changeMapSeq = undefined;
        this.appLang = "ja";
        this.backMap = undefined;
        this.mercSrc = undefined;
        ...
        this.from = undefined;
        this.lines = [];
        this.poiTemplate = false;
        this.poiStyle = false;
        this.iconTemplate = false;
    ...
    setMapClick() {
        this.mapObject.on("click", (evt) => {
            ...
            if (feature) {
                ...
                this.from.xy2MercAsync(xy).then((merc) => {
    ...
    raiseChangeViewPoint() {
        this.mapObject.on("postrender", (_evt) => {
            ...
            this.from
                .size2MercsAsync()
                .then((mercs) => this.mercSrc.mercs2SizeAsync(mercs))
                .then((size) => {
    ...
    setLine(data) {
        ...
        let xyPromises;
        if (data.coordinates) {
            xyPromises = data.coordinates.map((coord) => this.from.merc2XyAsync(coord));
        }
        else {
    ...
    convertParametersFromCurrent(to, callback) {
        const view = this.mapObject.getView();
        let fromPromise = this.from.size2MercsAsync();
