...
import { Tin } from '../node_modules/@maplat/tin/dist/tin.js';
...
async function generateCompiledForMap(mapPath) {
  ...
  try {
    ...
    const newMapPath = path.join(path.dirname(mapPath), `${mapName}.with-compiled.json`);
...
async function main() {
  ...
  const files = fs.readdirSync(mapsDir)
    .filter(file => file.endsWith('.json') && !file.includes('.compiled.') && !file.includes('.with-compiled.'))
  ...
  console.log('\nDone!');
  console.log('\nNote: The generated .with-compiled.json map files reference the compiled data.');
