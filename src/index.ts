...
export class MaplatApp extends EventTarget {
    ...
    constructor(appOption: any) {
        ...
        (app as any).logger = new Logger(appOption.debug ? LoggerLevel.ALL : LoggerLevel.INFO);
        ...
        if ((app as any).lang.toLowerCase() == "zh-hk" ||
        ...
        if (appOption.restore) {
            ...
            if (lastEpoch && currentTime - lastEpoch < 3600) {
                ...
                // @ts-expect-error ts-migrate(2345) FIXME: Argument of type 'string | 0' is not assignable to... Remove this comment to see the full error message
                (app as any).initialRestore.transparency = parseFloat(localStorage.getItem("transparency") || 0);
                (app as any).initialRestore.hideMarker = parseInt(localStorage.getItem("hideMarker") || "0")
                    ? true
                    : false;
                (app as any).initialRestore.hideLayer = localStorage.getItem("hideLayer");
            }
        }
        // Add UI HTML Element
    ...
    handleI18n(i18nObj: any, appOption: any) {
        ...
        return normalizeLayers((app as any).appData.pois || [], app).then(x => app.handlePois(x, mapReturnValue));
    }
    ...
    prepareMap(appOption: any) {
        ...
        (app as any).noRotate = appOption.noRotate || (app as any).appData.noRotate || false;
        (app as any).poiTemplate = appOption.poiTemplate || (app as any).appData.poiTemplate || false;
        (app as any).poiStyle = appOption.poiStyle || (app as any).appData.poiStyle || false;
    ...
    setInitialMap(cache: any) {
        const app = this;
        const initial = (app as any).initialRestore.mapID ||
    ...
    raiseChangeViewPoint() {
        ...
        app.mapObject.on("postrender", (_evt: any) => {
            ...
            app.from
                ...
                .then((size: any) => {
                if ((app as any).mobileMapMoveBuffer &&
    ...
    setLine(data: any) {
        ...
        let xyPromises;
        if (data.coordinates) {
            xyPromises = data.coordinates.map((coord: any) => app.from.merc2XyAsync(coord));
        }
        else {
    ...
    changeMap(mapID: any, restore: any) {
        ...
        return (app as any).changeMapSeq = (app as any).changeMapSeq.then(() => new Promise((resolve, _reject) => {
    ...
    requestUpdateState(data: any) {
        ...
        if ((app as any).timer)
            clearTimeout((app as any).timer);
        (app as any).timer = setTimeout(() => {
            (app as any).timer = undefined;
            app.dispatchEvent(new CustomEvent("updateState", (app as any).stateBuffer));
        }, 50);
    }
    ...
    convertParametersFromCurrent(to: any, callback: any) {
        ...
        (app as any).logger.debug(`From: Center: ${current[0]} Zoom: ${current[1]} Rotation: ${current[2]}`);
        ...
        fromPromise
            ...
            toPromise
                .then((size: any) => {
                (app as any).logger.debug(`To: Center: ${size[0]} Zoom: ${size[1]} Rotation: ${size[2]}`);
    ...
    translate(dataFragment: any) {
        const app = this;
        if (!dataFragment || typeof dataFragment != "object")
            return dataFragment;
        ...
        if ((app as any).i18n.exists(key, { ns: "translation", nsSeparator: "__X__yX__X__" }))
            return (app as any).t(key, { ns: "translation", nsSeparator: "__X__yX__X__" });
        for (let i = 0; i < langs.length; i++) {
            const lang = langs[i];
            (app as any).i18n.addResource(lang, "translation", key, dataFragment[lang]);
        }
        return (app as any).t(key, { ns: "translation", nsSeparator: "__X__yX__X__" });
    }
    remove() {
