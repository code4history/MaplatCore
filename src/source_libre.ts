...
import { HistMapLibreImpl } from "./source/histmap_libre_impl";
import "whatwg-fetch";
...
export async function mapSourceFactoryLibre(options: any, commonOptions?: any) {
  ...
  if (
    ...
    const targetSrc = NowMapLibre;
    
    options.homePosition = options.homePos || options.homePosition;
    options.mercZoom = options.defZoom || options.mercZoom;
      ...
      if (this.status === 200 || this.status === 0) {
        try {
          ...
          try {
            ...
            const obj = await HistMapLibreImpl.createAsync(options);
            resolve(obj);
          } catch (e) {
            reject(e);
          }
