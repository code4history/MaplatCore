...
test.describe('Maplat Basic Tests', () => {
  test.setTimeout(120000); // Increase timeout for Firefox
  ...
  test('should load test page', async ({ page }) => {
    ...
    await expect(page.locator('button#maplat')).toBeVisible({ timeout: 30000 });
    await expect(page.locator('button#osm')).toBeVisible({ timeout: 30000 });
  });
